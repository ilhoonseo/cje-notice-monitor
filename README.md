...
2. **이메일 실시간 즉시 알림 (SMTP)**:
   - 새로운 공지사항 발견 시 등록된 메일(`bjkanggr@gmail.com`)로 즉시 알림 메일 발송.
   - 깔끔한 HTML 기반 템플릿 메일로 공지 사항 제목, 작성자, 날짜 및 본문 600자 미리보기 텍스트와 원본 게시판 이동 버튼 제공.
   - 보안을 준수하여 이메일 계정 정보는 환경 변수(`SENDER_EMAIL`, `SENDER_PASSWORD`)로 안전하게 관리.
...
### 2. 환경 변수 구성
메일 발송 기능을 위해 로컬 터미널이나 시스템 환경 변수에 메일 계정 정보를 제공해야 합니다. (Gmail 계정 권장)

**💡 Gmail 앱 비밀번호 발급 방법**:
1. 발송용 Google 계정 로그인 후 [구글 계정 관리]로 이동합니다.
2. [보안] 탭의 'Google에 로그인하는 방법' 섹션에서 **2단계 인증**을 설정합니다.
3. 2단계 인증 설정 완료 후 맨 하단에 활성화되는 **앱 비밀번호 (App Passwords)** 메뉴를 클릭합니다.
4. 앱 이름을 `University Notice Alarmer` 등으로 자유롭게 설정한 후 [만들기]를 클릭하여 생성된 **16자리 비밀번호**를 복사합니다.

**로컬 터미널 세션 환경 변수 설정 (PowerShell)**:
```powershell
$env:SENDER_EMAIL="발송용이메일@gmail.com"
$env:SENDER_PASSWORD="복사한16자리앱비밀번호"
```
...
GitHub 프로젝트 리포지토리로 접속 후 다음 경로에 메일 정보를 등록합니다:
- **경로**: `Settings` -> `Secrets and variables` -> `Actions` -> `Repository secrets`
- **추가할 Secrets**:
  - `SENDER_EMAIL`: 발송에 사용할 이메일 주소 (예: `myname@gmail.com`)
  - `SENDER_PASSWORD`: 구글 계정에서 발급받은 **16자리 앱 비밀번호** (공백 없이 입력)
...
- 이메일의 ID와 암호(SENDER_EMAIL, SENDER_PASSWORD)는 절대 로컬 파일이나 소스 코드에 하드코딩해서는 안 됩니다.
- GitHub Actions 환경 변수(Secrets) 또는 Vercel Dashboard의 환경 변수 관리 도구를 이용하여 안전하게 보호되어 제공되어야 합니다.
